...
<a href='https://www.google.fr/search?q=linux+ufw'>Google search</a>
...
## OPTIONS
       ...
       default allow|deny|reject DIRECTION
              change the default policy for traffic going DIRECTION, where DI‐
              RECTION is one of incoming, outgoing or routed. Note that exist‐
       ...
       status show status of firewall and ufw managed rules. Use  status  ver‐
              ...
              that  when  using  status, there is a subtle difference when re‐
              porting interfaces. For example,  if  the  following  rules  are
              added:
...
RULE SYNTAX
       ...
       The above will allow all traffic to tcp ports 80, 443 and 8080-8090 in‐
       clusive  and  adds  a  comment  for  the rule. When specifying multiple
       ...
       normally allow the connection but will deny connections if  an  IP  ad‐
       dress attempts to initiate 6 or more connections within 30 seconds. See
       http://www.debian-administration.org/articles/187  for details. Typical
       usage is:

         ufw limit ssh/tcp

       Sometimes it is desirable to let the sender know when traffic is  being
       denied,  rather than simply ignoring it. In these cases, use reject in‐
       stead of deny.  For example:
...
REMOTE MANAGEMENT
       ...
       once ufw is 'enabled', ufw will not flush the chains when adding or re‐
       moving  rules  (but  will when modifying a rule or changing the default
...
APPLICATION INTEGRATION
       ...
       mand.  Users may also specify all to see the profiles for all known ap‐
       plications.
       ...
       be updated.  To update a profile and add a new rule to the firewall au‐
       tomatically, users can run:
...
REPORTS
       ...
       listening state for tcp and the open state for udp, along with the  ad‐
       dress of the interface and the executable listening on the port. An '*'
       ...
       listed  in  the  order  they are evaluated by the kernel, and the first
       match wins. Please note that the default policy is not listed and  tcp6
...
## NOTES
       ...
       for IPv4. destination-unreachable, packet-too-big, time-exceeded, para‐
       meter-problem, and echo-request for IPv6.

       - ACCEPT icmpv6 packets for stateless autoconfiguration (INPUT)

       - ACCEPT ping replies from IPv6 link-local (ffe8::/10) addresses (IN‐
       PUT)
